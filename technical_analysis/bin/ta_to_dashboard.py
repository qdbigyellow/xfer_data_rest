...
class Engine:
    def __init__(self, db_ip="192.168.0.6", action="cn"):
        ...
        self.symbol_list = SYMBOL_LIST.get(
            action, get_non_sp500_symbols(length=150))
        self.adx = Adx()
        self.rsi = Rsi()
        self.bbands = BBands()
    ...
    def exeucte(self, stock):
        ...
        if not "price" in data.keys():
            ...
            n_data = self.request_json(
                url=f"https://api.worldtradingdata.com/api/v1/stock?symbol={stock.symbol}&api_token={APIKEY.get('worldtradingdata')}")
    ...
    def update_table(self, stock, table):
        ...
        record_to_insert = (stock.symbol, stock.price,
                            stock.adx[0], stock.rsi[0], stock.bbands[0])
        insert_query(connection=db_conn, query=pg_insert_query,
