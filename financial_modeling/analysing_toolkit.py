    ...
    async with aiohttp.ClientSession() as session:
        ...
        for symbol in symbols:
            ...
            task = asyncio.create_task(
                download_financial_data(session, symbol))
            tasks.append(task)
...
async def download_financial_data(session, symbol, index=0):
    ...
    pb_ratio, pe_ratio, eps_growth, ebit_growth, fcf_growth = read_data(
        key_metrics, growth)
    ...
    """
    record_to_insert = (symbol, eps_growth, fcf_growth,
                        pb_ratio, pe_ratio, ebit_growth)
...
def read_data(key_metrics, growth, index=0):
    ...
    if len(key_metrics) > 0:
        pb_ratio = float(key_metrics[SupportKeys.pb_ratio.path][index][SupportKeys.pb_ratio.value]
                         ) if key_metrics[SupportKeys.pb_ratio.path][index][SupportKeys.pb_ratio.value] != "" else 0.0
        pe_ratio = float(key_metrics[SupportKeys.pe_ratio.path][index][SupportKeys.pe_ratio.value]
                         ) if key_metrics[SupportKeys.pe_ratio.path][index][SupportKeys.pe_ratio.value] != "" else 0.0
    if len(growth) > 0:
        ebit_growth = float(growth[SupportKeys.ebit_growth.path][index][SupportKeys.ebit_growth.value]
                            ) if growth[SupportKeys.ebit_growth.path][index][SupportKeys.ebit_growth.value] != "" else 0.0
        eps_growth = float(growth[SupportKeys.eps_growth.path][index][SupportKeys.eps_growth.value]
                           ) if growth[SupportKeys.eps_growth.path][index][SupportKeys.eps_growth.value] != "" else 0.0
        fcf_growth = float(growth[SupportKeys.free_cash_flow_growth.path][index][SupportKeys.free_cash_flow_growth.value]
                           ) if growth[SupportKeys.free_cash_flow_growth.path][index][SupportKeys.free_cash_flow_growth.value] != "" else 0.0
